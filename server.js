...
mcpServer.registerTool(
  ...
  async ({ action, value }) => {
    if (action === 'set' && (value === undefined || value === null)) {
      return {
        content: [{ type: 'text', text: 'A positive numeric value is required when action is "set"' }],
        isError: true
      };
    }
    ...
    return {
      content: [{ type: 'text', text: `Color scale threshold ${desc}` }]
    };
  }
);
